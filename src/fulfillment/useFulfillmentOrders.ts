...
import type { FulfillmentOrder, FulfillmentOrdersCursor } from '../types';
import { useFulfillmentManualReview } from './useFulfillmentManualReview';
...
export function useFulfillmentOrders(
  ...
  const manualReview = useFulfillmentManualReview(
    { walletAddress, enabled, dropIds: selectedDropIds },
    api.listFulfillmentManualReviewCheckouts,
  );
    ...
    const loadInitial = async () => {
      try {
        const responses = await Promise.all(selectedDropIds.map(async (dropId) => {
          const ordersResponse = await api.listFulfillmentOrders({ limit: FULFILLMENT_ORDER_REQUEST_LIMIT, cursor: null, dropId });
          return {
            dropId,
        ...
        dispatch({ type: 'initialLoaded', orders, cursorsByDropId });
      } catch (error) {
        if (!isCurrent()) return;
        console.error(error);
