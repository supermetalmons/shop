...
export default function WipInteractiveCard({ card }: { card: WipCardConfig }) {
  ...
  const [loadedCardSrc, setLoadedCardSrc] = useState<string | null>(null);
  const [interacting, setInteracting] = useState(false);
  const loading = loadedCardSrc !== card.imageSrc;
  const foilStyle = useMemo<React.CSSProperties>(
    () =>
      loading
        ? {}
        : {
  ...
  const baseCardStyle = useMemo<React.CSSProperties>(
    () => ({
      ['--pointer-x' as never]: '50%',
      ['--pointer-y' as never]: '50%',
      ['--card-scale' as never]: '1',
      ['--card-opacity' as never]: '0',
      ['--translate-x' as never]: '0px',
      ['--translate-y' as never]: '0px',
      ['--rotate-x' as never]: '0deg',
      ['--rotate-y' as never]: '0deg',
      ['--background-x' as never]: 'var(--pointer-x)',
      ['--background-y' as never]: 'var(--pointer-y)',
      ['--pointer-from-center' as never]: '0',
      ['--pointer-from-top' as never]: 'var(--pointer-from-center)',
      ['--pointer-from-left' as never]: 'var(--pointer-from-center)',
      ['--card-aspect' as never]: '0.718',
      ['--card-radius' as never]: '4.55% / 3.5%',
      ['--card-edge' as never]: 'hsl(47, 100%, 78%)',
      ['--card-back' as never]: 'hsl(205, 100%, 25%)',
      ['--card-glow' as never]: 'hsl(175, 100%, 90%)',
      ['--sunpillar-1' as never]: 'hsl(2, 100%, 73%)',
      ['--sunpillar-2' as never]: 'hsl(53, 100%, 69%)',
      ['--sunpillar-3' as never]: 'hsl(93, 100%, 69%)',
      ['--sunpillar-4' as never]: 'hsl(176, 100%, 76%)',
      ['--sunpillar-5' as never]: 'hsl(228, 100%, 74%)',
      ['--sunpillar-6' as never]: 'hsl(283, 100%, 73%)',
      ['--sunpillar-clr-1' as never]: 'var(--sunpillar-1)',
      ['--sunpillar-clr-2' as never]: 'var(--sunpillar-2)',
      ['--sunpillar-clr-3' as never]: 'var(--sunpillar-3)',
      ['--sunpillar-clr-4' as never]: 'var(--sunpillar-4)',
      ['--sunpillar-clr-5' as never]: 'var(--sunpillar-5)',
      ['--sunpillar-clr-6' as never]: 'var(--sunpillar-6)',
    }),
    [],
  );
  ...
  const onLayerLoad = useCallback(() => {
    setLoadedCardSrc((prev) => (prev === card.imageSrc ? prev : card.imageSrc));
  }, [card.imageSrc]);
  ...
  return (
    <div
      ref={cardRef}
      className={`drif-card ${card.glowType} interactive masked glowing${interacting ? ' interacting' : ''}${loading ? ' loading' : ''}`}
      data-number="001"
      data-set="custom"
      data-subtypes="v"
    ...
    >
      <div className="drif-card__translater">
        <button
          ...
          aria-label="Revealed Poncho Drifella card"
          tabIndex={0}
        >
          <div className="drif-card__back" aria-hidden="true" />
          <div className="drif-card__front" style={frontStyle}>
            <img src={card.imageSrc} alt="Revealed Poncho Drifella card" onLoad={onLayerLoad} width="1000" height="1400" draggable={false} />
