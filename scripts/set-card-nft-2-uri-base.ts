...
function parseArgs(argv: string[]): Options {
  ...
  let releaseDir = process.env.CARD_NFT_2_RELEASE_DIR?.trim()
...
function releaseIdentity(releaseDir: string): { elfBytes: number; elfSha256: string } {
  ...
  if (
    ...
    || manifest.baseImage !== VERIFIED_BUILD_IMAGE
...
async function readLiveState(
  ...
  const [genesis, programAccount, programDataAccount, balanceLamports, ...sharedConfigAccounts] = await Promise.all([
  ...
  for (let index = 0; index < SHARED_CONFIGS.length; index += 1) {
    ...
    if (!new PublicKey(account.data.subarray(8, 40)).equals(ADMIN)) {
...
function assertSiblingConfigsUnchanged(before: LiveState, after: LiveState): void {
  for (const address of SHARED_CONFIGS.slice(1)) {
    const key = address.toBase58();
    if (!before.sharedConfigData.get(key)?.equals(after.sharedConfigData.get(key)!)) {
      throw new Error(`Sibling config changed: ${key}`);
    }
  }
}
...
async function simulate(
  ...
  if (!data || data.length !== CONFIG_BYTES) throw new Error('Simulation did not return the 376-byte config');
...
async function main() {
  ...
  console.log('Card NFT 2 URI setter preflight passed.');
  ...
  try {
    ...
    for (let attempt = 1; attempt <= SEND_ATTEMPTS; attempt += 1) {
      ...
      if (recheck.config.uriBase === target) {
        ...
        assertSiblingConfigsUnchanged(before, recheck);
        break;
      }
      assertSiblingConfigsUnchanged(before, recheck);
    ...
    assertSiblingConfigsUnchanged(before, after);
    ...
    console.log('sibling configs same:', true);
  } finally {
    keyInput = '';
