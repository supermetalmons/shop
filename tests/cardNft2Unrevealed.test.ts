...
function firstNUnrevealedEligibleIds(count: number): number[] {
  ...
  for (let id = 1; id <= CARD_NFT_2_MAX_CARD_ID && ids.length < count; id += 1) {
    if (CARD_NFT_2_AS_GOOD_AS_SUPER_RARE_CARD_ID_SET.has(id)) continue;
    ids.push(id);
  }
...
test('card_nft_2 unrevealed candidates remove invalid ids, duplicates, and as-good ids', () => {
  ...
  assert.equal(result.includes(curatedId), false);
});
...
test('card_nft_2 unrevealed pagination scans sparse pools in id order after cursor', () => {
  ...
  assert.deepEqual(
    paginateCardNft2UnrevealedCandidateIds({
      rawPool: [fifth, 0, fourth, fourth, superRareId, first, pixelMosaicId, curatedId, second, 'not-a-card', third],
      limit: 2,
      cursor: second,
    }),
    {
