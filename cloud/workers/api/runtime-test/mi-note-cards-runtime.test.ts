...
import { MI_NOTE_2_CONTRACT_ADDRESS } from '../../../../shared/miNoteCards.js';
  ...
  try {
    ...
    context.mock.method(globalThis, 'fetch', async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return Response.json({
        ownedNfts: (cursor === null ? ['12', '2'] : ['4', '12']).map((tokenId) => ({
          contractAddress: MI_NOTE_2_CONTRACT_ADDRESS,
          tokenId,
          balance: '1',
        })),
    ...
    assert.deepEqual(await response.json(), { ok: true, tokenIds: ['2', '4', '12'] });
