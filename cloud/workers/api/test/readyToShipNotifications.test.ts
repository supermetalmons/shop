...
test('ready-to-ship outbox creates exact buyer and shipper jobs', async () => {
  const before = order({
    ...
    [BUYER_ORDER_RECEIVED_EMAIL_JOB_FIELD]: { obsolete: true },
    [SHIPPER_READY_TO_SHIP_EMAIL_QUEUED_AT_FIELD]: 100,
    [SHIPPER_READY_TO_SHIP_EMAIL_JOB_FIELD]: { obsolete: true },
  ...
  assert.deepEqual(outbox.values, {
    ...
    [BUYER_ORDER_RECEIVED_EMAIL_JOB_FIELD]: commerceFieldValue.delete(),
    ...
    [SHIPPER_READY_TO_SHIP_EMAIL_JOB_FIELD]: commerceFieldValue.delete(),
