...
export const MI_NOTE_CONTRACT_ADDRESSES = [MI_NOTE_3_CONTRACT_ADDRESS, MI_NOTE_2_CONTRACT_ADDRESS, MI_NOTE_CONTRACT_ADDRESS] as const;
export const MI_NOTE_CARDS_API_PATH = '/mi-note-cards';
export const MAX_MI_NOTE_TOKEN_IDS = 10_000;
export const MAX_MI_NOTE_RESPONSE_BYTES = 1024 * 1024;
...
export function isCanonicalMiNoteTokenIds(value: unknown): value is string[] {
...
export function isExactMiNoteCardsResponse(value: unknown): value is MiNoteCardsResponse {
  ...
  for (const contract of MI_NOTE_CONTRACT_ADDRESSES) {
    ...
    if (!isCanonicalMiNoteTokenIds(tokenIds) || !isRecord(result)) return false;
